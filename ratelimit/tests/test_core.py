...
class FunctionsTests(TestCase):
    ...
    def test_get_usage(self):
        _get_usage = partial(
            get_usage, method=get_usage.ALL, key="ip", rate="1/m", group="a"
        )
...
class RatelimitCBVTests(TestCase):
    ...
    def test_class_decorator(self):
        @method_decorator(ratelimit(key="ip", rate="1/m", block=False), name="get")
        class TestView(View):
...
class CacheFailTests(TestCase):
    ...
    @override_settings(
        RATELIMIT_USE_CACHE="connection-errors", RATELIMIT_FAIL_OPEN=True
    )
